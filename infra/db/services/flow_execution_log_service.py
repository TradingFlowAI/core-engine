...
from infra.db.postgres_manager import PostgresManager
...
class FlowExecutionLogService:
    ...
    async def delete_logs_before_cycle(self, flow_id: str, cycle: int) -> int:
        """
        Delete logs for a specific flow before the specified cycle.
        
        Args:
            flow_id: Flow identifier
            cycle: Delete logs before this cycle number
        ...
        try:
            ...
            logger.info(
                "Deleted %d flow execution logs for flow_id=%s before cycle %d",
                deleted_count,
                flow_id,
                cycle,
            )
            ...
            logger.error("Error deleting old flow execution logs: %s", str(e))
            raise
    ...
    async def get_logs_by_flow_cycle_node(
        ...
        try:
            with self.postgres_manager.get_session() as session:
                ...
                # Convert to dict format while session is still active
                logs_data = []
                for log in logs:
                    log_dict = {
                        'id': log.id,
