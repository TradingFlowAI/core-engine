...
@register_node_type(
    ...
    default_params={
        ...
        "header_row": True,
        "node_category": "input",
        "display_name": "Google Sheet Input",
        "version": "0.0.1",
    },
)
# 注册 Google Sheet Output 节点（写入模式）
@register_node_type(
    "gsheet_output_node",
    default_params={
        "spreadsheet_id": "",
        ...
        "display_name": "Google Sheet Output",
        "version": "0.0.1",
    },
)
class GSheetNode(NodeBase):
    """
    Google Sheets 节点 - 用于读取和写入 Google Sheets 数据

    输入参数:
    - spreadsheet_id: Google Sheets 文档 ID 或完整 URL
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.setdefault('display_name', 'Google Sheet Node')
    ...
    async def execute(self) -> bool:
        ...
        try:
            await self.persist_log(
                f"Starting GSheetNode execution: mode={self.mode}, spreadsheet_id={self.spreadsheet_id}, worksheet_name={self.worksheet_name}", "INFO"
            )
            ...
            error_msg = f"GSheetNode execution error: {str(e)}"
            await self.persist_log(error_msg, "ERROR")
            await self.set_status(NodeStatus.FAILED, error_msg)
            return False
        finally:
