...
class AptosVaultService:
    ...
    async def has_balance_manager(self, investor_address: str) -> bool:
        ...
        try:
            ...
            return result.get("hasBalanceManager", False)
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                return False
            else:
    ...
    async def get_investor_events(
        ...
        try:
            ...
            if event_type:
                ...
                if event_type_upper == "DEPOSIT":
                    operation_types = [OperationType.DEPOSIT]
                elif event_type_upper == "WITHDRAW":
                    operation_types = [OperationType.WITHDRAW]
                elif event_type_upper == "SWAP":
            ...
            with db_session() as db:
                ...
                for op in operations:
                    event = {
                        ...
                        "transaction_hash": str(op.transaction_hash) if op.transaction_hash else "",
                        "operation_type": op.operation_type.value,
                        "created_at": op.created_at.isoformat() + "Z" if op.created_at else "",
                        "updated_at": op.updated_at.isoformat() + "Z" if op.updated_at else "",
                    }

                    # 添加代币信息
                    if op.input_token_address:
                        event["input_token_address"] = op.input_token_address
                        event["input_token_amount"] = str(int(op.input_token_amount * 100000000)) if op.input_token_amount else "0"  # 转换为最小单位

                    if op.output_token_address:
                        event["output_token_address"] = op.output_token_address
                        event["output_token_amount"] = str(int(op.output_token_amount * 1000000)) if op.output_token_amount else "0"  # 转换为最小单位

                    events.append(event)

                logger.info(
                    f"Retrieved {len(events)} events for investor {investor_address} from database"
                )
            ...
            logger.error(f"Error getting investor events from database: {e}")
            raise
